...
const SEED = [
  {
    ...
    start_price: 8500,
    current_bid: 9200,
    end_time: new Date(Date.now() + 3600e3 * 24).toISOString(),
  },
  {
    brand: "Samsung",
    ...
    start_price: 7000,
    current_bid: 7650,
    end_time: new Date(Date.now() + 3600e3 * 48).toISOString(),
  },
  {
    brand: "Google",
    model: "Pixel 8 Pro",
    ...
    start_price: 5500,
    current_bid: 5800,
    end_time: new Date(Date.now() + 3600e3 * 12).toISOString(),
  },
  {
    brand: "OnePlus",
    model: "12R",
    ...
    start_price: 3200,
    current_bid: 3450,
    end_time: new Date(Date.now() + 3600e3 * 72).toISOString(),
  },
  {
    brand: "Apple",
    model: "iPhone 14",
    ...
    start_price: 5000,
    current_bid: 5300,
    end_time: new Date(Date.now() + 3600e3 * 36).toISOString(),
  },
  {
    brand: "Samsung",
    model: "Galaxy A54",
    ...
    start_price: 2800,
    current_bid: 2950,
    end_time: new Date(Date.now() + 3600e3 * 60).toISOString(),
  },
];
...
function Countdown({ endTime }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setLeft(new Date(endTime) - Date.now()), 1000);
    return () => clearInterval(t);
  }, [endTime]);
  const urgent = left < 3600e3 && left > 0,
    ended = left <= 0;
  return (
    <span
      style={{
        ...
        padding: "3px 9px",
        borderRadius: 20,
        background: ended ? C.border : urgent ? C.redLight : C.greenLight,
        color: ended ? C.textMuted : urgent ? C.red : C.green,
      }}
    >
      {ended ? "⏹ Ended" : `⏱ ${formatTime(left)}`}
    </span>
  );
}
...
function CondBadge({ cond }) {
  const map = {
    "Like New": [C.greenLight, C.green],
    Excellent: [C.blueLight, C.blue],
    Good: [C.warningLight, C.warning],
    Fair: [C.redLight, C.red],
  };
...
function ConfirmModal({ msg, onConfirm, onCancel }) {
  return (
    ...
    >
      <div
        style={{ ...s.card, maxWidth: 360, width: "90%", textAlign: "center" }}
      >
        <div style={{ fontSize: 24, marginBottom: 12 }}>🤔</div>
        <div style={{ fontSize: 15, marginBottom: 20 }}>{msg}</div>
        <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <button onClick={onCancel} style={s.btn}>
            Cancel
          </button>
          <button onClick={onConfirm} style={s.btnPrimary}>
            Confirm bid
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  async function loadBids(listingId) {
    try {
      const data = await sb(
        `bids?listing_id=eq.${listingId}&order=created_at.desc&select=*`
      );
  ...
  const toggleWatch = (id) => {
    ...
    showToast(
      next.includes(id) ? "Added to watchlist" : "Removed from watchlist"
    );
  };

  // ── Bid ───────────────────────────────────────────────────────────────────
  const activePhone = selectedId
  ...
  function requestBid() {
    ...
    if (!session) {
      showToast("Sign in to place a bid", "error");
      return;
    }
    if (!amt || isNaN(amt)) {
    ...
    if (amt <= activePhone.current_bid) {
      showToast(
        `Bid must exceed ${formatZAR(activePhone.current_bid)}`,
        "error"
      );
      return;
    }
    if (amt > activePhone.current_bid * 5)
      showToast("Unusually high bid — double check!", "warn");
  ...
  async function confirmBid() {
    ...
    try {
      ...
      await sb(`listings?id=eq.${phone.id}`, {
        method: "PATCH",
        headers: { ...HEADERS, Prefer: "return=minimal" },
        body: JSON.stringify({ current_bid: amount }),
      });
      setListings((prev) =>
        prev.map((p) => (p.id === phone.id ? { ...p, current_bid: amount } : p))
      );
  ...
  async function addListing() {
    ...
    if (!brand || !model || !startPrice || !hours) {
    ...
    try {
      await sb("listings", {
        ...
        body: JSON.stringify({
          ...
          current_bid: price,
          end_time: new Date(
            Date.now() + parseFloat(hours) * 3600e3
          ).toISOString(),
        }),
      });
      await loadListings();
  ...
  const filtered = listings.filter((p) => {
    ...
    const matchFilter =
      filter === "all" ||
      (filter === "active" && new Date(p.end_time) > Date.now()) ||
      (filter === "ending" &&
        new Date(p.end_time) - Date.now() < 3600e3 &&
        new Date(p.end_time) > Date.now()) ||
      (filter === "watched" && watchlist.includes(p.id));
  ...
  const navBtn = (key, label) => (
    <button
      ...
      style={{
        ...s.btn,
        border: "none",
        background: page === key ? C.blueLight : "transparent",
        color: page === key ? C.blue : C.textSecondary,
    ...
    >
      ...
      {confirm && (
        <ConfirmModal
          msg={
            <>
              Confirm bid of <b>{formatZAR(confirm.amount)}</b> on{" "}
              <b>
                {confirm.phone.brand} {confirm.phone.model}
              </b>
              ?
            </>
          }
          ...
          <>
            ...
            >
              <div style={{ fontSize: 28, fontWeight: 800, marginBottom: 6 }}>
                Bid on pre-owned phones 🇿🇦
              </div>
              <div style={{ fontSize: 15, opacity: 0.85 }}>
                Live auctions in South African Rand. No hidden fees.
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              {[
                ...
                ["active", "Active"],
                ["ending", "Ending soon"],
                ["watched", `Watchlist (${watchlist.length})`],
              ].map(([k, l]) => (
                <button
                  key={k}
                  onClick={() => setFilter(k)}
                  style={{
                    ...s.btn,
                    padding: "7px 14px",
                    background: filter === k ? C.blueLight : C.card,
                    color: filter === k ? C.blue : C.textSecondary,
              ...
              <span
                style={{ fontSize: 13, color: C.textMuted, marginLeft: "auto" }}
              >
                      ...
                      <div>
                        ...
                        <div
                          style={{
                            fontWeight: 800,
                            fontSize: 18,
                            color: ended ? C.textMuted : C.blue,
                          }}
                        >
                          {formatZAR(p.current_bid)}
                        </div>
                      </div>
              ...
              <div>
                ...
                >
                  <div style={{ ...s.card }}>
                    ...
                    >
                      {activePhone.image_url ? (
                        <img
                          src={activePhone.image_url}
                    ...
                    >
                      ...
                      <button
                        onClick={() => toggleWatch(activePhone.id)}
                        style={{
                          background: "none",
                          border: "none",
                          cursor: "pointer",
                      ...
                      >
                        {watchlist.includes(activePhone.id) ? "❤️" : "🤍"}
                      </button>
                    </div>
                    <div style={{ marginTop: 12 }}>
                      {[
                        ["Storage", activePhone.storage],
                        ["Condition", activePhone.condition],
                        ["Colour", activePhone.color],
                        ["Starting price", formatZAR(activePhone.start_price)],
                      ].map(([k, v]) => (
                        <div
                          key={k}
                          style={{
                        ...
                        >
                          ...
                          <span style={{ fontWeight: 500 }}>
                            {k === "Condition" ? <CondBadge cond={v} /> : v}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  >
                    <div style={s.card}>
                      ...
                      >
                        {formatZAR(activePhone.current_bid)}
                      </div>
                      {!ended &&
                        (session ? (
                          <>
                            <label style={s.label}>
                              Your bid (min {formatZAR(minBid)})
                            </label>
                            <input
                              type="number"
                              placeholder={minBid}
                            ...
                            >
                              You'll confirm before your bid is submitted.
                            </div>
                          </>
                        ) : (
                          <button
                          ...
                          >
                            ...
                            <span
                              style={{
                                ...
                                color: i === 0 ? C.blue : C.textPrimary,
                              }}
                            >
                              {formatZAR(b.amount)}
                            </span>
        ...
        {page === "profile" && session && (
          <div style={{ maxWidth: 640, margin: "0 auto" }}>
            ...
            <div style={s.card}>
              ...
              {watchlist.length === 0 ? (
                <div style={{ fontSize: 13, color: C.textMuted }}>
                  Nothing in your watchlist. Tap 🤍 on any listing.
                </div>
              ) : (
                watchlist.map((id) => {
              ...
              >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAdminForm((f) => ({ ...f, condition: e.target.value }))
                    }
                    style={s.inp}
                  >
              ...
              {listings.map((p) => (
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: C.textMuted }}>
                      {p.storage} · <CondBadge cond={p.condition} />
                    </div>
                  </div>
                  <div
                    style={{
